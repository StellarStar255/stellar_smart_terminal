...
- [排障：PowerShell + 代理下 claude login 失败 / PowerShell + proxy claude login](docs/troubleshooting-powershell-proxy-claude-login.md) — Windows 下代理要用 `$env:http_proxy = "..."` 而非 `set`，否则子进程 claude 拿不到代理导致 login 失败
